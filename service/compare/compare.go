 ...
 *初步检测完全一致则不需要进一步检测
 **/
func simpleDiff(jsonA map[string]interface{}, jsonB map[string]interface{}) bool {
	return reflect.DeepEqual(jsonA, jsonB)
}

func JsonCompareDiff(jsonA map[string]interface{}, jsonB map[string]interface{}, n int) (string, bool) {
	if simpleDiff(jsonA, jsonB) {
		return "",true
	}
	diff := &JsonDiff{HasDiff: false, Result: ""}

	jsonDiffDict(jsonA, jsonB, 1, diff)
	if diff.HasDiff {
		if n < 0 {
